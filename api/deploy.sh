...
cid=$(docker ps -a | grep 233cafe_app |awk '{print $1}')
echo $cid
if [ x"$cid" != x ]
    then
    docker rm -f $cid
...
docker build -t 233cafe_app .
echo "start container ..."
docker run -d --restart=always -p 8000:8000 \
-v /var/web/logs:/var/web/233cafe/logs --name 233cafe_app 233cafe_app
echo "current container ..."
docker ps -a | grep 233cafe_app
echo "clean docker ..."
